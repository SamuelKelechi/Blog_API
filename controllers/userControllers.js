...
const changeAvatar = async (req, res, next) => {
    try {
        ...
        if(avatar.size > 50000000) {
            return next(new HttpError("Image too large. File size should not exceed 5MB", 422))
        }
        ...
        avatar.mv(path.join(__dirname, '..', 'uploads', newFilename), async (err) => {
            ...
            if(!updatedAvatar) {
                return next(new HttpError("Image couldn't be changed", 422))
            }
...
const editUser = async (req, res, next) => {
    try {
        ...
        // Update other details without updating email
